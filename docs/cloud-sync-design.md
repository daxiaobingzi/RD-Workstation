...
### P2：体验与可见性（✅ 已落地）
- [x] 设置页 OSS 卡片：新增「立即同步」按钮、「最后同步时间 + 同步摘要（拉取/推送/冲突）」指示
- [x] 冲突记录列表：本机记录最近 50 条冲突（key/时间/胜者），每条可「查看归档」——弹窗只读展示 OSS `data/_conflicts/` 中的败者完整数据
- [x] 冲突 toast 提示（同步时若发现冲突即提示，可在系统配置查看）
